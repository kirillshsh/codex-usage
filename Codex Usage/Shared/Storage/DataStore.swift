...
enum MenuBarIconStyle: String, CaseIterable, Codable {
    ...
    var description: String {
        switch self {
        case .battery:
            return "Original battery-style bar with Codex text below"
        case .progressBar:
...
class DataStore: StorageProvider {
    ...
    func loadUsage() -> CodexUsage? {
        guard let data = defaults.data(forKey: Constants.UserDefaultsKeys.codexUsageData) else {
            LoggingService.shared.logStorageLoad("codexUsageData", success: false)
            return nil
        }

        do {
            let usage = try decoder.decode(CodexUsage.self, from: data)
            LoggingService.shared.logStorageLoad("codexUsageData", success: true)
            return usage
        } catch {
