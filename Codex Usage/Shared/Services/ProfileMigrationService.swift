...
class ProfileMigrationService {
    ...
    private func migrateCredentialsToProfile(_ profileId: UUID) throws {
        ...
        // Migrate Codex.ai session key from old Keychain location
        if let sessionKey = try? keychain.load(for: .codexSessionKey) {
            profiles[index].codexSessionKey = sessionKey
            LoggingService.shared.log("Migrated Codex session key")
        }
