...
//  Codex Usage
//
//  Created by Codex Code on 2026-01-11.
//
...
final class AutoStartSessionService {
    ...
    private init() {
        self.apiService = CodexAPIService()
    ...
    private func checkProfile(_ profile: Profile) async {
        // Skip if profile doesn't have Codex.ai credentials
        guard profile.hasCodexAI else {
            LoggingService.shared.logDebug("Skipping profile '\(profile.name)' - no Codex.ai credentials")
            return
        }
    ...
    private func fetchUsageForProfile(_ profile: Profile) async throws -> CodexUsage {
        ...
        await MainActor.run {
            profileManager.saveCodexUsage(usage, for: profile.id)
        }

        return usage
    }

    private func fetchUsageData(for profile: Profile) async throws -> CodexUsage {
        // Get credentials from the specific profile
        guard let sessionKey = profile.codexSessionKey,
        ...
        // Build URL
        let url = try URLBuilder(baseURL: Constants.APIEndpoints.codexBase)
    ...
    private func sendInitializationMessage(for profile: Profile) async throws -> Data {
        ...
        let conversationURL = try URLBuilder(baseURL: Constants.APIEndpoints.codexBase)
        ...
        let messageURL = try URLBuilder(baseURL: Constants.APIEndpoints.codexBase)
        ...
        let messageBody: [String: Any] = [
            "prompt": "Hi",
            "model": "codex-haiku-4-5-20251001",  // Ensures non-zero usage to prevent duplicate auto-starts
            "timezone": "UTC"
        ]
        ...
        let deleteURL = try URLBuilder(baseURL: Constants.APIEndpoints.codexBase)
