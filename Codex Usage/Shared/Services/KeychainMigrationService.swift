...
//  Codex Usage
//
//  Created by Codex Code on 2025-12-28.
//
...
class KeychainMigrationService {
    ...
    func performMigrationIfNeeded() {
        ...
        // 1. Migrate Codex.ai session key from file
        migratedCount += migrateCodexSessionKeyFromFile()
    ...
    /// Migrates Codex.ai session key from ~/.codex-session-key file to Keychain
    /// - Returns: 1 if migrated, 0 if not needed
    private func migrateCodexSessionKeyFromFile() -> Int {
        // Check if key already exists in Keychain
        if KeychainService.shared.exists(for: .codexSessionKey) {
            LoggingService.shared.log("Codex session key already in Keychain, skipping file migration")
            return 0
        }
        ...
        guard FileManager.default.fileExists(atPath: sessionKeyPath.path) else {
            LoggingService.shared.log("No Codex session key file found to migrate")
            return 0
        }
        ...
        do {
            ...
            guard validator.isValid(trimmedKey) else {
                LoggingService.shared.log("Codex session key in file is invalid, skipping migration")
                return 0
            }

            // Save to Keychain
            try KeychainService.shared.save(trimmedKey, for: .codexSessionKey)
            ...
            LoggingService.shared.log("Migrated Codex session key from file to Keychain")
            return 1

        } catch {
            LoggingService.shared.log("Failed to migrate Codex session key from file: \(error.localizedDescription)")
            return 0
        }
    }
