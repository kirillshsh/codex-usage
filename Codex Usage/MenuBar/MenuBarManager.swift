...
class MenuBarManager: NSObject, ObservableObject {
    ...
    func setup() {
        ...
        // Only when Codex snapshot data is available.
        ...
        networkMonitor.onNetworkAvailable = { [weak self] in
            ...
            // Skip if profile has no usage snapshot
        ...
        // Only if profile has usage snapshot data.
    ...
    private func fetchUsageForProfile(_ profile: Profile) async throws -> CodexUsage {
        ...
        throw AppError(
            ...
            message: "Codex usage snapshot not found for profile '\(profile.name)'",
            isRecoverable: true,
            recoverySuggestion: "Run codex usage tracker autostart and wait for the next snapshot."
        )
    }
    ...
    func refreshUsage() {
        ...
        // Check for Codex usage snapshot availability
